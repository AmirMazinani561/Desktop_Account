...
| مشخصه | مقدار |
|---|---|
| نسخه سند | ۰.۴ (**تأییدشده** — قطعی‌شدن نگاشت اسکیمای کیف پول و کدگذاری خودکار) |
| تاریخ | ۲۳ شهریور ۱۴۰۵ (2026-09-14) |
| سند بالادستی | `PRD.md` نسخه ۰.۵ (تأییدشده) |
| موتور هدف | PostgreSQL (تک‌موتوره) + ORM (Prisma) + مهاجرت SQL برای تریگرها/قیدها |
| وضعیت | ✅ تأیید کارفرما در ۱۴۰۵/۰۶/۲۳ + دریافت اسکیمای کامل `src/db/schema.ts` |
| تصمیم‌های قطعی‌شدهٔ این نسخه | واحد نمایش مبلغ = **ریال** • لینک کیف پول = **یک‌طرفه (INBOUND) + فقط افزایشی** • سال مالی قابل بستن/گشایش ✅ • ویرایش/حذف مجاز (VOID+جدید) ✅ • **کدگذاری خودکار حساب‌ها، اشخاص، بانک‌ها و اسناد** ✅ • نگاشت ستون‌به‌ستون کیف پول قطعی شد |
...
13. **کدگذاری و شماره‌گذاری ۱۰۰٪ خودکار (تصمیم قطعی کارفرما):** تمام حساب‌ها (درخت ۴ سطحی گروه/کل/معین/تفصیلی)، اشخاص و طرف‌حساب‌ها، حساب‌های بانکی و صندوق، شماره سریال اسناد، فاکتورها، چک‌ها و اسناد انبار به‌صورت **اتوماتیک** توسط توابع دیتابیسی (`fn_next_account_code`, `fn_next_counterparty_code`, `fn_next_serial`) شماره‌گذاری می‌شوند و کاربر نیازی به تعیین دستی کد ندارد.
...
| حذف/ویرایش تراکنش (در UI: ویرایش/حذف؛ در DB: ابطال+سند جدید با ممیزی) | ✅ | ✅ | ❌ |
...
### ۱۰.۱۱ اصلاح اشتباه در سند ثبت‌شده — ویرایش/حذف مجاز (۱۴۰۵/۰۶/۲۳ نوبت دوم)
کارفرما تأیید کرد ویرایش و حذف باید در UI ممکن باشد. برای حفظ تراز و حسابرسی، در سطح دیتابیس **UPDATE مستقیم روی `POSTED` همچنان ممنوع است** (تریگر رد می‌کند) و عملیات کاربر به‌صورت زیر ترجمه می‌شود:
۱) «ویرایش»: سند معکوسِ خودکار (همان آرتیکل‌ها با `side` جابه‌جا، `kind='ADJUSTMENT'`, `reverse_of_journal_id=<اصل>`, `description='ابطال و جایگزینی سند ۱۴۰۵/۰۰۰۱۲۳'`) + سند جدیدِ ویرایش‌شده — هر دو در یک تراکنش.
۲) «حذف»: فقط ابطال (VOID) با `void_reason` + ثبت در `audit_logs`؛ ردیف حذف فیزیکی هرگز انجام نمی‌شود (قید `chk_journals_soft_delete_draft_only`).
۳) دسترسی: OWNER و ACCOUNTANT مجاز، VIEWER محروم (ماتریس ۳.۳)؛ هر رویداد در `audit_logs` با `action='UPDATE'/'VOID'` ثبت می‌شود.
این طراحی هم خواستهٔ «ویرایش/حذف ممکن» را برآورده می‌کند، هم اصل حسابداری «اسناد ثبت‌شده تغییرناپذیرند» را نمی‌شکند.
...
## ۱۱. مهاجرت داده و نگاشت دقیق از «کیف پول هوشمند» (بر مبنای `src/db/schema.ts` واقعی)

> ✅ **وضعیت اسکیمای کیف پول:** فایل `src/db/schema.ts` شامل ۳ جدول (`users`, `accounts`, `transactions`) بررسی شد.
> اگرچه طبق تصمیم کارفرما **فقط داده‌های جدید به‌صورت افزایشی** از روز فعال‌سازی منتقل می‌شوند و مهاجرت تاریخچه در MVP الزامی نیست، نگاشت زیر به‌صورت **قطعی و ستون‌به‌ستون** مستند شده تا هم برای اتصال افزایشی (API فاز ۲) و هم برای ابزار بازیابی بکاپ یکسان و پایدار عمل کند.

### ۱۱.۱ نگاشت ستون‌به‌ستون جدول `users`
| ستون مبدأ (`legacy.users`) | ستون مقصد (`users`) | تبدیل / منطق |
|---|---|---|
| `id varchar(64)` | `id UUID` | تبدیل به UUID معتبر؛ ثبت در نگاشت شناسه‌ها |
| `username varchar(150)` | `email` و `email_lower` | اگر قالب ایمیل نبود: `username@wallet.local`؛ تریگر `email_lower` را پر می‌کند |
| `password_hash text` | `password_hash` | حفظ هش برای امکان ورود کاربر بدون نیاز به بازنشانی رمز |
| `full_name varchar(191)` | `display_name` | انتقال مستقیم؛ پیش‌فرض «کاربر کیف پول» در صورت NULL |
| `created_at / updated_at` | `created_at / updated_at` | انتقال مستقیم |
| *(رکورد جانبی خودکار)* | `companies` + `memberships` | ساخت مجموعهٔ پیش‌فرض (`base_unit='RIAL'`) + عضویت با `role_kind='OWNER'` |

---

### ۱۱.۲ نگاشت ستون‌به‌ستون جدول `accounts`
| ستون مبدأ (`legacy.accounts`) | ستون مقصد (`accounts` + `counterparties`) | تبدیل / منطق |
|---|---|---|
| `id varchar(64)` | `id UUID` | تبدیل به UUID معتبر |
| `type varchar(20)` (`bank`) | `account_class='ASSET'`, `account_kind='BANK'` | والد: معین بانک‌ها (`10102`)؛ کد خودکار `10102001`, ... |
| `type varchar(20)` (`cash`) | `account_class='ASSET'`, `account_kind='CASH'` | والد: معین صندوق‌ها (`10101`)؛ کد خودکار `10101001`, ... |
| `type varchar(20)` (`person`) | `account_class='ASSET'`, `account_kind='PERSON'` + ساخت رکورد در `counterparties` | ساخت طرف‌حساب با `type='NATURAL'`, `name=name` و اتصال ۱:۱ با `linked_account_id`؛ والد: بدهکاران تجاری (`10201`) |
| `type varchar(20)` (`income`) | `account_class='INCOME'`, `account_kind='INCOME_HEADING'` | والد: ریشه درآمدهای عملیاتی (`40101`)؛ کد خودکار `40101001`, ... |
| `type varchar(20)` (`expense`) | `account_class='EXPENSE'`, `account_kind='EXPENSE_HEADING'` | والد: ریشه هزینه‌های عمومی (`50101`)؛ کد خودکار `50101001`, ... |
| `name varchar(191)` | `name` | انتقال نام حساب |
| `initial_balance doublePrecision` | `opening_balance BIGINT` | ضرب در ضریب ریال (گرد به نزدیک‌ترین عدد صحیح ریالی)؛ `opening_side='DEBIT'` |
| `is_favorite boolean` | `is_favorite` | انتقال مستقیم |
| `is_parent boolean` | `is_postable = NOT is_parent` | سرفصل‌های اصلی `is_postable=false` و سرفصل‌های برگ `is_postable=true` |
| `parent_id varchar(64)` | `parent_id UUID` | اتصال به والد مربوطه؛ تریگر `trg_accounts_tree` مسیر `path` و `depth` را خودکار می‌سازد |
| `detail_info text` | `bank_account_no` / `card_no_last4` / `notes` | استخراج شماره کارت/حساب بانکی (اگر در قالب JSON/متن بود) یا نگهداری در توضیحات |
| `icon varchar(64)` / `color varchar(32)` | `icon` / `color` | انتقال مستقیم برای نمایش در UI |
| `created_at / updated_at` | `created_at / updated_at` | انتقال مستقیم |

---

### ۱۱.۳ نگاشت ستون‌به‌ستون جدول `transactions`
| ستون مبدأ (`legacy.transactions`) | ستون مقصد (`journals` + `journal_lines`) | تبدیل / منطق |
|---|---|---|
| `id varchar(64)` | `journals.id UUID` | تبدیل به UUID |
| `type` (`expense`/`income`/`transfer`) | `journals.kind = 'IMPORT'` (یا `'TRANSACTION'`) | نوع در سیستم جدید از روی طبیعت حساب‌ها مشتق می‌شود |
| `amount doublePrecision` | `journal_lines.amount BIGINT` (ریال صحیح) | گرد کردن اعشار به عدد صحیح ریالی |
| `fee doublePrecision` | سطر کارمزد در `journal_lines` | اگر `fee > 0`: سطر ۳ بدهکار حساب «هزینه کارمزد بانکی» (`50109001`) با `line_role='FEE'` |
| `from_account_id` | سطر ۱ `journal_lines` | `side='CREDIT'`, `line_role='SOURCE'`, `amount = amount + fee` (بستانکار مبدأ) |
| `to_account_id` | سطر ۲ `journal_lines` | `side='DEBIT'`, `line_role='DESTINATION'`, `amount = amount` (بدهکار مقصد) |
| `date timestamp` | `journals.date DATE` | تاریخ میلادی مبنا |
| `shamsi_date varchar(12)` | `journals.jalali_year / month / day` | استخراج سال/ماه/روز شمسی (مثلاً `1403/05/12` ← ۱۴۰۳، ۵، ۱۲) |
| `description text` | `journals.description` | شرح سند |
| `tracking_number varchar(120)` | `journals.ref_no` + `ref_kind='BANK_TRACE'` | کد پیگیری بانکی برای جستجوی یکپارچه |
| *(تولید سیستمی)* | `journals.serial_no` | تولید شماره سریال بی‌شکاف با `fn_next_serial` در همان سال مالی |
| *(تولید سیستمی)* | `journals.status` | `status = 'POSTED'`, `posted_at = date` |
...
## ۱۶. پرسش‌های کارفرما (۷ مورد پاسخ داده شد، ۱ مورد توضیحی باقی)
...
2. ✅ **پاسخ داده شد (۱۴۰۵/۰۶/۲۳ نوبت دوم): سال مالی قابل بستن و گشایش سال جدید.** کارفرما تأیید کرد سال مالی باید قابلیت «بسته شدن» و «ورود به سال مالی جدید» داشته باشد. پیاده‌سازی: `fiscal_years.status = CLOSED/LOCKED` + سند اختتامیه/افتتاحیه + قفل دوره (بند ۴.۶)؛ سال جدید با `jalali_year+1` و بازهٔ جدید ساخته می‌شود.
3. ✅ **پاسخ داده شد (۱۴۰۵/۰۶/۲۳ نوبت دوم): حذف و ویرایش تراکنش باید امکان‌پذیر باشد.** تصمیم طراحی: در UI دکمهٔ «ویرایش/حذف» برای نقش‌های **OWNER و ACCOUNTANT** فعال است (VIEWER ندارد)، ولی در سطح دیتابیس **ویرایش مستقیم `POSTED` مجاز نیست**؛ عملیات به‌صورت «ابطال (VOID) سند قبلی + ثبت سند جدید (ADJUSTMENT)» با `reverse_of_journal_id` و ثبت در `audit_logs` انجام می‌شود تا تراز و حسابرسی حفظ شود (بند ۱۰.۱۱). ماتریس ۳.۳ به‌روزرسانی شد.
4. ✅ **پاسخ داده شد (۱۴۰۵/۰۶/۲۳ نوبت دوم): ثبت خودکار پیامک نمی‌خواهد.** همیشه با تأیید کاربر؛ گزینهٔ «ثبت خودکار برای بانک با نگاشت قطعی» پیاده‌سازی نمی‌شود (ساده‌سازی فاز ۱).
5. ✅ **پاسخ داده شد (۱۴۰۵/۰۶/۲۳ نوبت دوم): انتقال کیف پول فقط افزایشی از روز فعال‌سازی.** هیچ تراکنش تاریخی وارد نمی‌شود؛ فقط داده‌های جدید پس از ران شدن نرم‌افزار. نگاشت مهاجرت بند ۱۱ به‌عنوان «گزینهٔ یدکی» باقی می‌ماند.
6. ℹ️ **«دسترسی به کد قدیم یعنی چی؟» — توضیح:** برای اینکه نگاشت دقیق هر فیلد قدیم → جدید (مثلاً ستون‌های `src/db/schema.ts` و ساختار JSON بکاپ کیف پول) را بدون حدس بنویسیم، به **محتوای `src/db/schema.ts` + یک نمونه فایل بکاپ JSON (بدون `.env`)** نیاز داریم. درخت فایل ارسالی (۱۴۰۵/۰۶/۲۳) دریافت شد و نشان داد استک `Next.js + Drizzle + pg` است؛ ولی بدون دیدن خود `schema.ts` نمی‌توان ستون‌به‌ستون را قطعی کرد. بند ۱۱ تا آن زمان «پیشنهادی» می‌ماند. ارسال امن: `cat src/db/schema.ts` یا آپلود همان فایل در `legacy/` همین مخزن (هرگز `.env` نفرستید).
7. ✅ **پاسخ داده شد (۱۴۰۵/۰۶/۲۳ نوبت دوم): چندزبانه/چندارزی نمی‌خواهد.** تأیید شد هیچ‌کدام در دامنه نیست؛ ستون‌های ارزی فعلاً اضافه نمی‌شوند (بند ۱۲).

---

## ۱۷. قدم بعدی

1. ✅ انجام شد: تأیید این سند و PRD توسط کارفرما (۱۴۰۵/۰۶/۲۳) + پاسخ‌های نوبت دوم (۱۴۰۵/۰۶/۲۳).
2. تصمیم استراتژی کد (ردیف ۵ جدول تصمیم‌های PRD) — درخت فایل کیف پول رسید (Next.js + Drizzle)؛ منتظر `src/db/schema.ts` برای نهایی‌سازی (گزینهٔ الف محتمل).
3. ادامهٔ **فاز ۰**: `schema.prisma` ✅ + `db/sql/0001_*.sql` ✅ → بعدی: `db/seed/system_accounts.sql` + `lib/jalali.ts` + تست‌های طلایی ۱۰.۱–۱۰.۴ و تست‌های رد.
4. لینک افزایشی کیف پول (فاز ۲): `POST /api/v1/external/transactions` با `idempotency_key` و `occurred_at >= فعال‌سازی`؛ بدون مهاجرت تاریخچه.

---

*پایان نسخه ۰.۴ — تأییدشده توسط کارفرما؛ نگاشت دقیق اسکیمای کیف پول و کدگذاری خودکار نهایی شد.*
