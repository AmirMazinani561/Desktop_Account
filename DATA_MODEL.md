...
| مشخصه | مقدار |
|---|---|
| نسخه سند | ۰.۳ (**تأییدشده** — به‌روزرسانی پس از پاسخ‌های ۱۴۰۵/۰۶/۲۳ نوبت دوم) |
| تاریخ | ۲۳ شهریور ۱۴۰۵ (2026-09-14) |
| سند بالادستی | `PRD.md` نسخه ۰.۴ (تأییدشده) |
| موتور هدف | PostgreSQL (تک‌موتوره) + ORM (Prisma) + مهاجرت SQL برای تریگرها/قیدها |
| وضعیت | ✅ تأیید کارفرما در ۱۴۰۵/۰۶/۲۳ + پاسخ‌های نوبت دوم |
| تصمیم‌های قطعی‌شدهٔ این نسخه | واحد نمایش مبلغ = **ریال** • لینک کیف پول = **یک‌طرفه (INBOUND) + فقط افزایشی از روز فعال‌سازی** • سال مالی قابل بستن/گشایش ✅ • ویرایش/حذف تراکنش مجاز (VOID+جدید) ✅ • عدم ثبت خودکار پیامک ✅ • بدون چندزبانه/چندارزی ✅ |
...
> ⚠️ **وابستگی باز + تصمیم جدید:** کد/اسکیمای نسخهٔ قدیم هنوز دیده نشده؛ نگاشت زیر بر اساس تحلیل PRD نوشته شده و **پس از دیدن `src/db/schema.ts`** قطعی می‌شود. طبق تصمیم ۱۴۰۵/۰۶/۲۳ نوبت دوم، **مهاجرت تاریخچه انجام نمی‌شود** و فقط تراکنش‌های جدید پس از راه‌اندازی به‌صورت افزایشی وارد می‌شوند؛ این بند به‌عنوان گزینهٔ یدکی (در صورت نیاز به مهاجرت دستی یک فایل بکاپ خاص) نگه داشته شده است.
...
*پایان نسخه ۰.۳ — تأییدشده توسط کارفرما (شامل پاسخ‌های نوبت دوم ۱۴۰۵/۰۶/۲۳)؛ مبنای ادامهٔ فاز ۰.*
