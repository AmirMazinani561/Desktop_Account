...
| سند بالادستی | `PRD.md` نسخه ۰.۲ |
| موتور هدف | PostgreSQL (تک‌موتوره) + ORM (Prisma) + مهاجرت SQL برای تریگرها/قیدها |
| وضعیت | ⏳ در انتظار تأیید کارفرما — تا تأیید، کد اجرایی نوشته نمی‌شود |
...
این سند بر پایهٔ چهار پیشنهاد PRD نوشته شده است. اگر کارفرما هرکدام را رد کند، بخش «اثر تغییر تصمیم» (بند ۱۳) نشان می‌دهد چه چیزی باید عوض شود.
...
| ۳ | مبالغ = **عدد صحیح ریالی (BIGINT)** | هیچ `float`/`double` در ستون پول وجود ندارد |
...
| `base_unit` | VARCHAR(10) | NOT NULL DEFAULT `'RIAL'` — فقط **نمایش** (`RIAL`/`TOMAN`) |
...
| `direction` | VARCHAR(10) | `INBOUND`/`OUTBOUND`/`BOTH` — MVP فقط INBOUND |
...
## ۱۶. پرسش‌های باز برای کارفرما

1. **واحد نمایش پیش‌فرض:** ریال یا تومان؟ (ذخیره همیشه ریال است؛ فقط نمایش)
...
5. **لینک کیف پول:** یک‌طرفه (فقط دریافت از کیف پول) یا دوطرفه (نوشتن تراکنش جدید در کیف پول هم)؟ کدام نوع تراکنش‌ها منتقل شوند؟
...
1. تأیید این سند (نسخه ۰.۱) + پاسخ پرسش‌های بخش ۱۶ + تأیید PRD نسخه ۰.۲.
...
*پایان نسخه ۰.۱ — این سند مکمل `PRD.md` است و تا تأیید کارفرما مبنای کدنویسی قرار نمی‌گیرد.*
